...
@interface YHULrcView() <UITableViewDataSource>
...
{
    // 1.创建Cell
    YHULrcCell *cell = [YHULrcCell lrcCellWithTableView:tableView];
    ...
    // 2.给cell设置数据
    // 2.1.取出模型
    YHULrcLine *lrcline = self.lrclist[indexPath.row];
...
- (void)setLrcName:(NSString *)lrcName
{
    ...
    // 2.解析歌词
    self.lrclist = [YHULrcTool lrcToolWithLrcName:lrcName];
...
- (void)setCurrentTime:(NSTimeInterval)currentTime
{
    ...
    for (int i = 0; i < count; i++) {
        // 1.拿到i位置的歌词
        YHULrcLine *currentLrcLine = self.lrclist[i];
        
        // 2.拿到下一句的歌词
        NSInteger nextIndex = i + 1;
        YHULrcLine *nextLrcLine = nil;
        if (nextIndex < count) {
        ...
        if (self.currentIndex == i) {
            // 4.1.拿到i位置的cell
            NSIndexPath *indexPath = [NSIndexPath indexPathForRow:i inSection:0];
            YHULrcCell *cell = (YHULrcCell *)[self.tableView cellForRowAtIndexPath:indexPath];
...
- (void)generatorLockImage
{
    ...
    // 2.拿到三句歌词
    // 2.1.获取当前的歌词
    YHULrcLine *currentLrc = self.lrclist[self.currentIndex];
    // 2.2.上一句歌词
    NSInteger previousIndex = self.currentIndex - 1;
    YHULrcLine *prevousLrc = nil;
    if (previousIndex >= 0) {
        prevousLrc = self.lrclist[previousIndex];
    }
    // 2.3.下一句歌词
    NSInteger nextIndex = self.currentIndex + 1;
    YHULrcLine *nextLrc = nil;
