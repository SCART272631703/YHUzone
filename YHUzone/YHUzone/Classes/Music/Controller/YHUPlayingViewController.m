...
@interface YHUPlayingViewController () <UIScrollViewDelegate, AVAudioPlayerDelegate>
...
@implementation YHUPlayingViewController
...
{
    // 1.取出当前播放歌曲
    YHUMusic *playingMusic = [YHUMusicTool playingMusic];
    ...
    // 3.开始播放歌曲
    AVAudioPlayer *currentPlayer = [YHUAudioPlayerTool playMusicWithMusicName:playingMusic.filename];
...
- (IBAction)previous {
    // 1.取出上一首歌曲
    YHUMusic *previousMusic = [YHUMusicTool previousMusic];
...
- (IBAction)next {
    // 1.取出下一首歌曲
    YHUMusic *nextMusic = [YHUMusicTool nextMusic];
    
    // 2.播放下一首歌曲
    [self playingMusicWithMusic:nextMusic];
}

- (void)playingMusicWithMusic:(YHUMusic *)music
{
    // 1.停止当前歌曲
    YHUMusic *playingMusic = [YHUMusicTool playingMusic];
    [YHUAudioPlayerTool stopMusicWithMusicName:playingMusic.filename];
    
    // 3.播放歌曲
    [YHUAudioPlayerTool playMusicWithMusicName:music.filename];
    
    // 4.将工具类中的当前歌曲切换成播放的歌曲
    [YHUMusicTool setPlayingMusic:music];
